    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md bg-white">
        ...
        <div className="space-y-4 py-2">
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                maxLength={4}
                className="flex-1 rounded-none border border-slate-200 px-3 py-2 text-sm font-mono uppercase tracking-wider focus:border-blue-500 focus:outline-none"
          ...
          {preview && (
            <div className="rounded-none border border-slate-200 bg-slate-50 p-3 space-y-1">
              <div className="text-[11px] font-bold text-slate-900">{preview.title}</div>
              {preview.organism && (
                <div className="text-[10px] text-slate-500 italic">{preview.organism}</div>
              )}
              <div className="flex gap-3 text-[9px] text-slate-400 font-mono">
                {preview.method && <span>{preview.method}</span>}
                {preview.resolution != null && <span>{preview.resolution.toFixed(2)} A</span>}
                {preview.length != null && <span>{preview.length} residuos</span>}
              </div>
            </div>
          )}
        </div>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => handleClose(false)} className="text-[10px]">
            Cancelar
          </Button>
          <Button
