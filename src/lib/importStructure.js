import { looksLikeCif } from './molstar/structurePipeline'
...
function extractOrganismFromCif(text) {
  const match = text.match(/_entity_src_gen\.pdbx_organism_scientific_name\s+(.+)/)
  if (match) return match[1].trim().replace(/^['"]|['"]$/g, '')
  const match2 = text.match(/_entity_src_nat\.pdbx_organism_scientific_name\s+(.+)/)
...
export function parseStructureFile(text, fileName, formatHint = null) {
  ...
  const organism = isCif ? extractOrganismFromCif(text) : extractOrganismFromPdb(text)
