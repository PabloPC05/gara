...
export function FileMenu() {
  ...
  const labelClass =
    "px-2 py-1.5 text-[10px] font-black uppercase tracking-widest text-slate-500";
  const separatorClass = "bg-white/10 mx-1 my-1";
  const contentClass =
    "w-[360px] bg-[#111113] border border-white/10 shadow-2xl backdrop-blur-md rounded-xl p-1.5";
  ...
  const handleRcsbImport = useCallback(
    async (pdbId, format) => {
      const { text, format: actualFormat } = await fetchStructureById(
        pdbId,
        format,
      );
      const protein = parseStructureFile(
        text,
        `${pdbId}.${format === "cif" ? "cif" : "pdb"}`,
      );
      protein.pdbId = pdbId.toLowerCase();
      protein.name =
        protein.name === pdbId
          ? (await fetchMetadataById(pdbId)).title
          : protein.name;
      upsertProtein(protein);
      setActiveProteinId(protein.id);
    },
  ...
  const handleExportPdb = useCallback(() => {
    ...
    const pdbFile =
      protein?._raw?.structural_data?.pdb_file ??
      protein?.pdbData ??
      protein?.structureData;
    if (!pdbFile) {
      alert("No hay datos estructurales para exportar.");
      return;
    }
    const name = (protein?.name || "protein")
  ...
  const handleExportFasta = useCallback(() => {
    ...
    if (!seq) {
      alert("No hay secuencia para exportar.");
      return;
    }
    const name = (protein?.name || "protein")
      .replace(/[^a-z0-9_-]+/gi, "_")
  ...
  const handleExportImage = useCallback(
    async ({ format, scale, transparent }) => {
  ...
  const handleExportPdf = useCallback(async () => {
    const id = selectedProteinIds[0];
    if (!id) {
      alert("Selecciona una proteína primero.");
      return;
    }
    const protein = proteinsById[id];
    if (!protein) return;
    ...
    <>
      ...
      <MenubarMenu>
        ...
        <MenubarContent className={contentClass}>
          <MenubarLabel className={labelClass}>
            Importar Estructura
          </MenubarLabel>
