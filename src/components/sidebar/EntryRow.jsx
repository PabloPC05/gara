...
export function EntryRow({ index, protein, isActive, onToggleSelection }) {
  ...
  return (
    <>
      <div
        className={`group flex items-center w-full rounded-none border px-3 py-2 transition-colors ${
          isActive
      ...
      >
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            {meta ? (
              ...
              <p className="mt-1 truncate text-[11px] text-slate-400">{protein?.id}</p>
            )}
          </div>
        </button>
        ...
        <button
          ...
          className="ml-2 shrink-0 rounded-md p-1 text-slate-300 hover:bg-red-50 hover:text-red-500 dark:text-slate-600 dark:hover:bg-red-950/20 dark:hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 focus-visible:opacity-100"
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            ...
            <DialogDescription>
              ¿Seguro que quieres eliminar <strong className="text-foreground">{label}</strong>?
