import type { RawProtein, NormalizedProtein } from "./proteinTypes";
import type { BiologicalDataOutput, SequenceProperties } from "@/api/types";

function toDalton(mw: number | null | undefined): number | null {
	if (mw == null) return null;
	return mw < 1000 ? mw * 1000 : mw;
}

export function normalizeProtein(
	protein: RawProtein,
	...
	const meta =
		raw.protein_metadata ??
		({} as Partial<import("@/api/types").ProteinMetadata>);
	const bioRaw = raw.biological_data ?? ({} as Partial<BiologicalDataOutput>);
	const seqProps =
		raw.sequence_properties ??
		bioRaw.sequence_properties ??
		({} as Partial<SequenceProperties>);
	const conf =
		raw.structural_data?.confidence ??
		({} as Partial<import("@/api/types").ConfidenceData>);
	const bioUnified = protein.biological ?? null;

	function pick<T>(...values: (T | null | undefined)[]): T | null {
		for (const v of values)
			if (v != null && v !== ("" as unknown as T)) return v;
		return null;
	}
	...
	const mwDa = toDalton(
		pick(
			...
			seqProps.molecular_weight_kda != null
				? seqProps.molecular_weight_kda * 1000
				: null,
		),
	);
	...
	return {
		...
		mwDa,
		isoelectricPoint: pick(
			protein.isoelectric_point,
			bioUnified?.isoelectricPoint,
		),
		positiveCharges: pick(
			bioUnified?.positiveCharges,
			seqProps.positive_charges,
		),
		negativeCharges: pick(
			bioUnified?.negativeCharges,
			seqProps.negative_charges,
		),
		...
		solubilityLabel: pick(
			bioUnified?.solubilityLabel,
			bioRaw.solubility_prediction,
		),
		instabilityIndex: pick(
			bioUnified?.instabilityIndex,
			bioRaw.instability_index,
		),
		instabilityLabel: pick(
			bioUnified?.instabilityLabel,
			bioRaw.stability_status,
		),
		...
		allergenicityAlerts: Array.isArray(bioRaw.allergenicity_alerts)
			...
			: [],
		knownStructures: Array.isArray(protein.known_structures)
			? protein.known_structures
			: [],
