...
export function SearchSection() {
  ...
  useEffect(() => {
    ...
    const loadInitialCatalog = async () => {
      ...
      try {
        ...
        if (cancelled) return
        setInitialCatalogError(error?.message ?? 'No se pudo consultar el catalogo')
      } finally {
        if (!cancelled) {
          setIsLoadingInitialCatalog(false)
        }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (searchId !== latestFilteredSearchIdRef.current) return
        setFilteredSearchError(error?.message ?? 'No se pudo consultar el catalogo')
      } finally {
      ...
      return (
        <div className="flex-1 overflow-auto mt-4 border border-slate-200 rounded-none bg-slate-50 p-4 flex flex-col items-center justify-center text-center text-slate-500">
          ...
          <p className="font-medium text-slate-700">
            {isFilteredMode ? 'Consultando catalogo...' : 'Cargando catalogo...'}
          </p>
          <p className="mt-2 text-xs text-slate-500">
            La primera peticion puede tardar hasta 30 s si la API estaba en reposo.
          </p>
        </div>
      )
    }
  ...
  return (
    <div className="flex flex-col gap-3 text-sm text-slate-700 h-full">
      ...
      {/* Job status panel from sidebar-nuevo */}
      <PersistentJobStatusPanel panelKey={JOB_PANEL_KEYS.catalog} />

      {/* Load error indicator (from vibracion) */}
      {loadError ? (
