...
const num3 = new Intl.NumberFormat('es-ES', { maximumFractionDigits: 3 })
  ...
  return (
    <div className="flex flex-col gap-0.5 py-1">
      ...
      >
        {value}
        {unit && (
          <span className="ml-0.5 text-[9px] font-normal text-slate-400">{unit}</span>
        )}
        {sub && (
...
export function PiChargeWidget({ v }) {
  ...
  return (
    <section className="py-2.5">
      ...
      <div className="flex flex-col divide-y divide-slate-100 border-y border-slate-100">
        {mwKda != null && (
          <Cell
            ...
            value={num2.format(mwKda)}
            unit="kDa"
            sub={`${intFmt.format(Math.round(mwDa))} Da`}
          />
        )}
        {avgRes != null && (
          <Cell label="Masa/residuo" value={num2.format(avgRes)} unit="Da/aa" />
        )}
        {pI != null && (
          <Cell
            label="Punto isoeléctrico (calc.)"
            value={num2.format(pI)}
            unit="pI"
            sub={pINote}
            tone={pITone}
          />
      ...
      <div className="mt-3 rounded-none border border-slate-200 bg-slate-50/40 p-3">
        <div className="mb-2 flex items-center justify-between">
          ...
          {pI != null && (
            <span className="font-mono text-[9px] text-slate-500">
              pI = <span className={pITone}>{num2.format(pI)}</span>
            </span>
          )}
        </div>
      ...
      <div className="mt-3 rounded-none border border-slate-200 bg-white p-3">
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">
          <div className="rounded-none border border-slate-100 px-2 py-1.5 text-center">
            <div className="text-[8px] uppercase tracking-wider text-slate-400">pH</div>
            <div className="font-mono text-[12px] font-bold tabular-nums text-slate-800">
              {num2.format(sliderPh)}
            </div>
          </div>
          <div className="rounded-none border border-slate-100 px-2 py-1.5 text-center">
            <div className="text-[8px] uppercase tracking-wider text-slate-400">Carga</div>
            <div className={`font-mono text-[12px] font-bold tabular-nums ${chargeTone}`}>
              {chargeAtSlider > 0 ? '+' : ''}
              {num2.format(chargeAtSlider)}
            </div>
          </div>
          <div className="rounded-none border border-slate-100 px-2 py-1.5 text-center">
      ...
      <div className="mt-3">
        ...
        <div className="mt-1 grid grid-cols-4 gap-x-2 gap-y-1">
          {ionizableList.map(({ key, label, pka }) => {
            ...
            return (
              <div
                key={key}
                className="flex items-baseline gap-1 rounded-none border border-slate-100 px-1.5 py-1"
              >
                <span
        ...
        <div className="mt-1 flex gap-2 text-[8px] text-slate-400 font-mono">
          <span>pKa: Asp 3.65 · Glu 4.25 · His 6.00 · Cys 8.18 · Tyr 10.07 · Lys 10.53 · Arg 12.48</span>
        </div>
      </div>
    </section>
  )
