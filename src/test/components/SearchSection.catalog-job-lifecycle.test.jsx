...
} from "@/components/left-sidebar/sections/CatalogSearchSection";
...
describe("CatalogSearchSection catalog job lifecycle", () => {
  beforeEach(() => {
  ...
  it("keeps status in the store across remounts using the real loader flow", async () => {
    ...
    mockPollJob.mockImplementation(
      (_, { onStatusChange } = {}) =>
        new Promise((resolve) => {
    ...
    const { unmount } = render(<CatalogSearchSection />);
    ...
    expect(
      useJobStatusStore.getState().panelsByKey[JOB_PANEL_KEYS.catalog]?.status,
    ).toBe("RUNNING");

    unmount();
    render(<CatalogSearchSection />);
    ...
    await waitFor(() => {
      expect(
        useJobStatusStore.getState().panelsByKey[JOB_PANEL_KEYS.catalog],
      ).toBeUndefined();
    });
  });
});
