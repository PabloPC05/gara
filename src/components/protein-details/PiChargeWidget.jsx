...
export function PiChargeWidget({ v }) {
  ...
  if (!sequence || !computed) return null
  ...
  const sliderMarkerData = useMemo(() => {
    return profile.map((p) => ({
      ...p,
      sliderCharge: p.pH === Math.round(sliderPh * 4) / 4 ? chargeAtSlider : null,
    }))
  }, [profile, sliderPh, chargeAtSlider])
  ...
  const totalPos = (ionizable.Lys || 0) + (ionizable.Arg || 0) + (ionizable.His || 0)
  const totalNeg = (ionizable.Asp || 0) + (ionizable.Glu || 0)

  return (
    <section style={{ maxWidth: '100%', overflow: 'hidden', paddingTop: 10, paddingBottom: 10 }}>
      ...
      <div className="mt-3 rounded-none border border-slate-200 bg-slate-50/40 p-3 overflow-hidden min-w-0">
        ...
        <ResponsiveContainer width="100%" height={160}>
          ...
          >
            ...
            <XAxis
              ...
              tick={{ fontSize: 8, fontFamily: 'ui-monospace, monospace', fill: '#94a3b8' }}
      ...
      <div className="mt-3 rounded-none border border-slate-200 bg-white p-3 overflow-hidden min-w-0">
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className={`font-mono text-[11px] font-bold tabular-nums ${chargeTone}`}>
            {chargeAtSlider > 0 ? '+' : ''}
            {num2Fmt.format(chargeAtSlider)} e{' '}
            <span className="text-[9px] font-normal text-slate-400">({chargeLabel})</span>
          </span>
        </div>
        ...
        <input
          ...
          className="w-full h-1.5 bg-gradient-to-r from-blue-200 via-slate-200 to-rose-200 rounded-none appearance-none cursor-pointer accent-slate-800
            [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-3.5 [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:rounded-none [&::-webkit-slider-thumb]:bg-slate-800 [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-white [&::-webkit-slider-thumb]:shadow-md
            [&::-moz-range-thumb]:h-3.5 [&::-moz-range-thumb]:w-3.5 [&::-moz-range-thumb]:rounded-none [&::-moz-range-thumb]:bg-slate-800 [&::-moz-range-thumb]:border-2 [&::-moz-range-thumb]:border-white [&::-moz-range-thumb]:shadow-md"
        />
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">
          <div className="rounded-none border border-slate-100 px-2 py-1.5 text-center overflow-hidden min-w-0">
            <div className="text-[8px] uppercase tracking-wider text-slate-400">pH</div>
            <div className="font-mono text-[12px] font-bold tabular-nums text-slate-800 truncate">
              {num2Fmt.format(sliderPh)}
            </div>
          </div>
          <div className="rounded-none border border-slate-100 px-2 py-1.5 text-center overflow-hidden min-w-0">
            <div className="text-[8px] uppercase tracking-wider text-slate-400">Carga</div>
            <div className={`font-mono text-[12px] font-bold tabular-nums truncate ${chargeTone}`}>
              {chargeAtSlider > 0 ? '+' : ''}
              {num2Fmt.format(chargeAtSlider)}
            </div>
          </div>
          <div className="rounded-none border border-slate-100 px-2 py-1.5 text-center overflow-hidden min-w-0">
            <div className="text-[8px] uppercase tracking-wider text-slate-400">Migración</div>
            <div className="text-[10px] font-bold tabular-nums text-slate-700 truncate">
      ...
      <div className="mt-3">
        ...
        <div className="mt-1 grid grid-cols-3 gap-x-2 gap-y-1 min-w-0">
          {ionizableList.map(({ key, label, pka }) => {
            ...
            return (
              <div
                key={key}
                className="flex items-baseline gap-1 rounded-none border border-slate-100 px-1.5 py-1 overflow-hidden min-w-0"
              >
                <span
        ...
        <div className="mt-1 text-[8px] text-slate-400 font-mono leading-relaxed overflow-hidden min-w-0">
          <span className="break-all">pKa: Asp 3.65 · Glu 4.25 · His 6.00 · Cys 8.18 · Tyr 10.07 · Lys 10.53 · Arg 12.48</span>
        </div>
      </div>
    </section>
  )
