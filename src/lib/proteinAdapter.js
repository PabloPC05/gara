...
function detectStructureFormat(data) {
	...
	if (
		/^(HEADER|TITLE {1,2}|COMPND|SOURCE|EXPDTA|AUTHOR|REMARK|SEQRES|ATOM {2}|HETATM|MODEL {1,2}|TER {3}|ENDMDL|END)/m.test(
			data,
		)
	) {
		return "pdb";
...
const instabilityLabelFromIndex = (index) =>
	index < 40 ? "Estable" : "Inestable";
	...
	return {
		...
		biological_data: bio
			? {
					...
					toxicity_alerts:
						bio.toxicityAlerts ??
						(bio.toxicityAlert ? [bio.toxicityLabel] : []),
		...
		sequence_properties: {
			...
			molecular_weight_kda:
				bio?.molecularWeightKda ??
				(bio?.molecularWeight ? bio.molecularWeight / 1000 : 0),
...
export function apiToUnified(validated, originalFasta = null) {
	...
	const plddtMean =
		meta.plddtAverage ?? structural.confidence?.plddtMean ?? null;
	...
	return {
		...
		biological: biological
			? {
					...
					solubilityLabel: solubilityLabelFromPrediction(
						biological.solubilityPrediction,
					),
					instabilityIndex: biological.instabilityIndex ?? 0,
					instabilityLabel:
						biological.stabilityStatus &&
						biological.stabilityStatus !== "unknown"
							? biological.stabilityStatus
		...
		fasta: originalFasta,
		sequence:
			extractSequenceFromFasta(originalFasta) ||
			extractSequenceFromPdb(structure.data),
