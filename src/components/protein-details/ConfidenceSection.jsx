  ...
  return (
    <section>
      ...
      <div className="flex flex-col gap-2">
        ...
        {meanPae != null && (
          <div className="flex items-center gap-3 rounded-xl border border-slate-200/60 bg-white/60 backdrop-blur-sm px-4 py-3">
...
function PlddtCard({ value }) {
  ...
  return (
    <div className={`flex flex-1 items-center gap-3 rounded-xl border ${tone.border} ${tone.bg} px-4 py-3 backdrop-blur-sm`}>
      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${tone.bg}`}>
        <span className={`h-3 w-3 rounded-full ${tone.ring} shadow-sm`} />
      </div>
      <div className="flex flex-1 flex-col gap-1.5">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    type="button"
                    className="inline-flex h-4 w-4 items-center justify-center rounded-full text-slate-300 hover:text-slate-500 transition-colors cursor-pointer"
                  >
        ...
        <div className="h-1.5 w-full rounded-full bg-white/60">
          <div
            className={`h-full rounded-full ${tone.ring} transition-all duration-500`}
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
