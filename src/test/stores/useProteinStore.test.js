...
describe("useProteinStore", () => {
	...
	describe("upsertProtein", () => {
		...
		it("updates an existing protein", () => {
			...
			useProteinStore
				.getState()
				.upsertProtein({ ...mockProtein(), name: "Updated" });

			expect(useProteinStore.getState().proteinsById["test-1"].name).toBe(
				"Updated",
			);
		});
		...
		it("ignores entries without id", () => {
			...
			expect(Object.keys(useProteinStore.getState().proteinsById)).toHaveLength(
				0,
			);
		});
	...
	describe("replaceCatalog", () => {
		...
		it("filters out entries without id", () => {
			useProteinStore
				.getState()
				.replaceCatalog([mockProtein(), { name: "No ID" }, mockProtein2()]);
	...
	describe("selection", () => {
		it("setSelectedProteinIds caps at 2 and sets active", () => {
			useProteinStore.getState().setSelectedProteinIds(["a", "b", "c"]);

			const state = useProteinStore.getState();
			expect(state.selectedProteinIds).toEqual(["a", "b"]);
			expect(state.activeProteinId).toBe("b");
		});
