  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        onClick={handleExport}
        disabled={status === 'loading'}
        className={`flex items-center justify-center gap-2 px-4 py-2 rounded-xl border font-medium transition-all duration-200 shadow-sm ${buttonStyles[status]}`}
      >
        {status === 'loading' ? (
          <Loader2 className="w-4 h-4 animate-spin" />
        ) : status === 'success' ? (
      ...
      {status === 'success' && exportResult && (
        <div className="bg-slate-50 border border-slate-200 p-3 flex flex-col gap-2 rounded-xl animate-in fade-in slide-in-from-top-2 duration-300">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Archivos creados:</p>
          <div className="grid grid-cols-1 gap-1.5">
            <a href={exportResult.folderUrl} target="_blank" rel="noreferrer" className="flex items-center justify-between group px-2 py-1.5 bg-white border border-slate-100 hover:border-blue-200 transition-all">
              <div className="flex items-center gap-2">
                <FolderOpen className="w-3.5 h-3.5 text-blue-500" />
                <span className="text-xs font-medium text-slate-700">Ver carpeta en Drive</span>
              </div>
              <ExternalLink className="w-3 h-3 text-slate-300 group-hover:text-blue-400" />
            </a>
            {exportResult.files.map((file, i) => (
              <a key={i} href={file.url} target="_blank" rel="noreferrer" className="flex items-center justify-between group px-2 py-1.5 bg-white border border-slate-100 hover:border-blue-200 transition-all">
                <div className="flex items-center gap-2">
