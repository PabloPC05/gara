...
function addSectionTitle(doc, title, y, gap = 5) {
	doc.setFontSize(10);
...
function ensurePage(doc, y, threshold = 240) {
	if (y > threshold) {
		doc.addPage();
		return 20;
	}
...
function addIdentitySection(doc, v, y) {
	y = addSectionTitle(doc, "Identidad", y);
...
function addMetricsSection(doc, v, y) {
	...
	y = addSectionTitle(doc, "Metricas Estructurales", y);

	const rows = [];
	if (v.plddtMean != null)
		rows.push(["pLDDT Medio", `${v.plddtMean.toFixed(2)}`]);
	if (v.meanPae != null) rows.push(["PAE Medio", `${v.meanPae.toFixed(2)} A`]);
	if (v.mwDa != null)
		rows.push([
			"Peso Molecular",
			`${(v.mwDa / 1000).toFixed(2)} kDa (${Math.round(v.mwDa)} Da)`,
		]);
	if (v.isoelectricPoint != null)
		rows.push(["Punto Isoelectrico", `${v.isoelectricPoint.toFixed(2)}`]);
	if (v.solubilityScore != null)
		rows.push([
			"Solubilidad",
			`${v.solubilityScore}/100 (${v.solubilityLabel || ""})`,
		]);
	if (v.instabilityIndex != null)
		rows.push([
			"Indice de Inestabilidad",
			`${v.instabilityIndex.toFixed(2)} (${v.instabilityLabel || ""})`,
		]);
	if (v.positiveCharges != null)
		rows.push(["Cargas (+)", `${v.positiveCharges}`]);
	if (v.negativeCharges != null)
		rows.push(["Cargas (-)", `${v.negativeCharges}`]);
...
function addSequenceSection(doc, v, y) {
	...
	y = ensurePage(doc, y, 240);
	y = addSectionTitle(doc, "Secuencia", y);
...
export async function exportProteinPdf(protein, plugin, options = {}) {
	...
	const seqProps =
		raw.sequence_properties ?? raw.biological_data?.sequence_properties ?? {};
	...
	const v = {
		...
		solubilityLabel:
			bioUnified?.solubilityLabel ?? bioRaw.solubility_prediction,
	...
	if (includeImage && plugin?.canvas3d) {
		try {
			...
			viewerImage = await captureViewerImage(plugin, {
				format: "png",
				scale: 2,
			});
		} catch (e) {
	...
	if (heatmapImage) {
		...
		y = addSectionTitle(doc, "Mapa de Error PAE", y, 3);
		const hmSize = Math.min(120, 290 - y - 10);
		doc.addImage(heatmapImage, "PNG", 14, y, hmSize, hmSize);
		y += hmSize + 8;
	}

	addSequenceSection(doc, v, y);

	addFooter(doc);

	const safeName = (v.name || "protein")
