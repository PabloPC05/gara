...
export function validateFasta(input: string): FastaValidation {
  ...
  if (!trimmed.startsWith(">")) {
    return {
      valid: false,
      reason: 'El FASTA debe empezar con ">" seguido de un encabezado.',
    };
  }
  ...
  if (!sequence)
    return {
      valid: false,
      reason: "Falta la secuencia después del encabezado.",
    };
  if (trimmed.length > 100_000)
    return {
      valid: false,
      reason: "Secuencia demasiado larga (máx. 100.000 caracteres).",
    };
