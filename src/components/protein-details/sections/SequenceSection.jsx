...
import { intFmt, safeFilename, downloadBlob } from '../formatters'
import { ChemicalBackbone } from './ChemicalBackbone'
...
export function SequenceSection({ v }) {
    ...
    <Section title="Secuencia" aside={`${intFmt.format(sequence.length)} aa`}>
      ...
      {showChem && (
        <div className="mt-2 overflow-x-auto border border-slate-200 bg-white">
          <ChemicalBackbone sequence={sequence} />
        </div>
      )}
    </Section>
  )
