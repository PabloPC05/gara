...
export const JOB_RESOURCE_PRESETS = [
  {
    id: 'quick',
    label: 'Vista rápida',
    description: 'Perfil ligero y equivalente a la configuración base de la API.',
    resources: {
      gpus: 0,
      cpus: 1,
      memory_gb: 4,
  ...
  },
  {
    id: 'standard',
    label: 'Estándar',
    description: 'Equilibrado para la mayoría de secuencias y pruebas normales.',
    resources: {
      gpus: 1,
      cpus: 8,
      memory_gb: 32,
...
export function getJobResourcesPreset(presetId) {
  return (
    ...
    ?? JOB_RESOURCE_PRESETS.find((preset) => preset.id === DEFAULT_JOB_RESOURCES_PRESET_ID)
    ?? JOB_RESOURCE_PRESETS[0]
  )
}

export function normalizeJobResources(resources = {}) {
  const fallback = getJobResourcesPreset(DEFAULT_JOB_RESOURCES_PRESET_ID).resources

  return {
    gpus: clampNumber(resources.gpus ?? fallback.gpus, JOB_RESOURCE_LIMITS.gpus),
    cpus: clampNumber(resources.cpus ?? fallback.cpus, JOB_RESOURCE_LIMITS.cpus),
    memory_gb: clampNumber(resources.memory_gb ?? fallback.memory_gb, JOB_RESOURCE_LIMITS.memory_gb),
...
export function resolveJobResourcesPresetId(resources) {
  ...
  const matchedPreset = JOB_RESOURCE_PRESETS.find((preset) => (
