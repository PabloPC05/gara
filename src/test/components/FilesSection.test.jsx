...
vi.mock('@/lib/proteinLoadService', () => ({
  loadProteinFromInputWithJobPanel: (...args) => mockLoadProteinFromInputWithJobPanel(...args),
}))
...
describe('FilesSection job status persistence', () => {
  beforeEach(() => {
  ...
  it('keeps showing RUNNING after remount until the job completes', async () => {
    ...
    mockLoadProteinFromInputWithJobPanel.mockImplementation(() => new Promise((resolve) => {
      useJobStatusStore.getState().upsertJobPanel(JOB_PANEL_KEYS.filesFasta, {
        status: 'RUNNING',
        error: null,
        jobId: 'job-1',
      })
      resolveLoad = () => {
        useJobStatusStore.getState().clearJobPanel(JOB_PANEL_KEYS.filesFasta)
        resolve('protein-1')
      }
    }))

    const { unmount } = render(<FilesSection />)
  ...
  it('keeps FAILED visible until dismissed', async () => {
    ...
    mockLoadProteinFromInputWithJobPanel.mockImplementation(() => new Promise((_, reject) => {
      useJobStatusStore.getState().upsertJobPanel(JOB_PANEL_KEYS.filesFasta, {
        status: 'RUNNING',
        error: null,
        jobId: 'job-1',
