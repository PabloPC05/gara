...
import { pubmedUrl } from "../utils/dataFormatters";

export function IdentityAndTaxonomySection({ v }) {
  const links = [
    v.uniprotId && {
      label: "UniProt",
  ...
  return (
    ...
    >
      ...
      <h2 className="mt-0.5 min-w-0 break-words text-base font-bold leading-tight tracking-tight text-slate-900">
        {v.name ?? "Proteína sin identificar"}
      </h2>

      <div className="mt-0.5 flex min-w-0 flex-wrap items-center gap-x-1.5 text-[11px] text-slate-500">
        {v.organism && <span className="italic">{v.organism}</span>}
        {v.organism && v.length != null && (
        ...
        <div className="mt-1.5 flex flex-wrap gap-1">
          {v.tags.map((tag) => (
            <Badge
              key={tag}
              variant="outline"
              className="rounded-none border border-slate-200 bg-slate-50/60 px-1.5 py-0.5 font-sans text-[9px] font-medium text-slate-600"
            >
              {tag}
            </Badge>
          ))}
