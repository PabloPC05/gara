...
export function ProteinDetailsDrawer() {
  ...
  useEffect(() => {
    const overlay = drawerRef.current?.parentElement
    if (!overlay) return
    const w = detailsPanelOpen ? drawerWidth : COLLAPSED_WIDTH
    overlay.style.setProperty('--details-width', `${w}px`)
    overlay.style.setProperty('--details-sidebar-width', `${drawerWidth}px`)
    overlay.style.setProperty('--details-sidebar-collapsed-width', `${COLLAPSED_WIDTH}px`)
  ...
  const handleResizeStart = useCallback((e) => {
    ...
    const onMove = (ev) => {
      ...
      if (wrapper) {
        ...
        wrapper.style.setProperty('--details-sidebar-width', `${w}px`)
      }
    }
    const onUp = () => {
  ...
  return (
    <>
      ...
      <div
        ...
        className="absolute right-0 top-0 bottom-0 z-40 flex flex-col overflow-hidden border-l border-slate-200 bg-white shadow-xl transition-transform duration-300 ease-in-out pointer-events-auto"
      >
        {/* Resize handle */}
        <div
      ...
      <div
        ...
        className="absolute right-0 top-0 bottom-0 z-30 flex flex-col items-center gap-2 border-l border-slate-200 bg-white py-4 px-1 cursor-pointer hover:bg-slate-50 transition-opacity duration-300 ease-in-out group pointer-events-auto"
