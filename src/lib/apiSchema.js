...
function validateConfidence(data) {
  if (!isObject(data)) return null
  return {
    plddtPerResidue: Array.isArray(data.plddt_per_residue) ? data.plddt_per_residue.map(Number) : [],
    ...
    paeMatrix: Array.isArray(data.pae_matrix)
      ? data.pae_matrix.map((row) => (Array.isArray(row) ? row.map(Number) : []))
      : [],
...
function validateBiologicalData(data) {
  ...
  return {
    ...
    toxicityAlerts: Array.isArray(data.toxicity_alerts) ? data.toxicity_alerts : [],
    allergenicityAlerts: Array.isArray(data.allergenicity_alerts) ? data.allergenicity_alerts : [],
    secondaryStructure: validateSecondaryStructure(data.secondary_structure_prediction),
