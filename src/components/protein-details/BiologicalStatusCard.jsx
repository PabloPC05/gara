import { CircleCheck, TriangleAlert, ShieldCheck, Droplets, Scale } from 'lucide-react'
import { SectionLabel } from './SectionLabel'

function StatusRow({ icon: Icon, label, value, score, tone }) {
  const isOk = tone === 'ok'
  return (
    <div className={`flex items-center gap-3 border px-4 py-3 ${isOk ? 'border-emerald-200/60 bg-emerald-50/50' : 'border-rose-200/60 bg-rose-50/50'}`}>
      <div className={`flex h-8 w-8 shrink-0 items-center justify-center ${isOk ? 'bg-emerald-100' : 'bg-rose-100'}`}>
        <Icon className={`h-4 w-4 ${isOk ? 'text-emerald-600' : 'text-rose-600'}`} strokeWidth={2.5} />
      </div>
      <div className="flex flex-1 items-center justify-between gap-3 min-w-0">
        <div className="flex flex-col min-w-0">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{label}</span>
          {score != null && <span className="text-[10px] text-slate-500 tabular-nums">{typeof score === 'number' ? score.toFixed(1) : score}</span>}
        </div>
        <span className={`text-[11px] font-black shrink-0 ${isOk ? 'text-emerald-700' : 'text-rose-700'}`}>{value}</span>
      </div>
    </div>
  )
}
...
function AlertsRow({ label, alerts }) {
  ...
  if (!hasAlerts) {
    return (
      <div className="flex items-center gap-3 border border-emerald-200/60 bg-emerald-50/50 px-4 py-3">
  ...
  return (
    <div className="border border-rose-200/60 bg-rose-50/50 px-4 py-3 overflow-hidden">
      <div className="flex items-center gap-3 mb-2">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center bg-rose-100">
          <TriangleAlert className="h-4 w-4 text-rose-600" strokeWidth={2.5} />
        </div>
        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{label}</span>
      </div>
      <ul className="ml-11 flex flex-col gap-1 overflow-hidden">
        {alerts.map((alert, i) => (
          <li key={i} className="text-[11px] font-medium text-rose-700 leading-relaxed truncate">{alert}</li>
        ))}
      </ul>
    </div>
  )
  ...
  return (
    <section className="overflow-hidden">
      ...
      <div className="flex flex-col gap-2">
        <StatusRow icon={Droplets} label="Solubilidad" value={solubilityPrediction} score={solubilityScore} tone={isSoluble ? 'ok' : 'warn'} />
        <StatusRow icon={Scale} label="Estabilidad" value={stabilityStatus} score={instabilityIndex} tone={isStable ? 'ok' : 'warn'} />
