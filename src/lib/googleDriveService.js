 ...
 * - Google Identity Services SDK cargado via <script> en index.html
 */
...
function getAuthHeaders(token, contentType = "application/json") {
...
const getValidToken = () => {
	return new Promise((resolve, reject) => {
		if (!tokenClient) {
			reject(
				new Error(
					"Google Identity Client no inicializado. Llama a initGoogleIdentity primero.",
				),
			);
			return;
		}
 ...
 */
export const exportResultsToWorkspace = async ({
	proteinName,
	summaryText,
	paeJsonData,
	metrics,
	...
	const folder = await createFolder(
		`BioHack - ${proteinName} - ${new Date().toLocaleDateString()}`,
		token,
	);
	...
	if (metrics || paeJsonData) {
		try {
			const sheet = await createGoogleSheet(
				`Métricas: ${proteinName}`,
				metrics || paeJsonData,
				folderId,
				token,
			);
			results.files.push({
				name: "Hoja de Métricas",
...
async function createGoogleDoc(title, content, parentId, token) {
	...
	if (!createResponse.ok) {
		throw new Error(
			`Error al crear documento: ${createResponse.status} ${createResponse.statusText}`,
		);
	}
	...
	const batchResponse = await fetch(
		`${DOCS_API_URL}/${docFile.id}:batchUpdate`,
		{
			method: "POST",
			headers: getAuthHeaders(token),
	...
	if (!batchResponse.ok) {
		throw new Error(
			`Error al insertar contenido en Doc: ${batchResponse.status}`,
		);
	}
...
async function createGoogleSheet(title, data, parentId, token) {
	...
	if (!createResponse.ok) {
		throw new Error(
			`Error al crear hoja: ${createResponse.status} ${createResponse.statusText}`,
		);
	}
	...
	if (rows.length > 0) {
		await fetch(
			`${SHEETS_API_URL}/${sheetFile.id}/values/A1:append?valueInputOption=USER_ENTERED`,
			{
				method: "POST",
				headers: getAuthHeaders(token),
...
async function uploadFile({ name, mimeType, content, parentId, token }) {
	...
	const boundary = `boundary_${crypto.randomUUID?.() ?? Date.now()}`;
	...
	const uploadResponse = await fetch(
		UPLOAD_API_URL.replace(
			"uploadType=multipart",
			`uploadType=multipart&boundary=${boundary}`,
		),
		{
			method: "POST",
			headers: {
	...
	if (!uploadResponse.ok) {
		throw new Error(`Error al subir archivo a Drive: ${uploadResponse.status}`);
	}
}
