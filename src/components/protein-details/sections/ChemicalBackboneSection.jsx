...
import { intFmt } from "../utils/dataFormatters";
import { RES_WIDTH, CHAIN_HEIGHT, MAX_RENDERED_RESIDUES } from "../utils/proteinConstants";
...
export const ChemicalBackboneSection = memo(function ChemicalBackboneSection({ sequence }) {
  const residues = sequence.slice(0, MAX_RENDERED_RESIDUES);
  const width = residues.length * RES_WIDTH + 60;

  return (
    <>
      <div className="flex min-w-0 items-center justify-between overflow-hidden border-b border-slate-100 px-2 py-1 font-mono text-[10px] text-slate-500">
      ...
      <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 overflow-hidden border-t border-slate-100 px-2 py-1 font-mono text-[9px] text-slate-500">
        <span className="inline-flex items-center gap-1">
          <span className="inline-block h-0.5 w-4 bg-amber-500" />
          enlace peptídico
        </span>
        <span className="inline-flex items-center gap-1">
          <span className="inline-block h-2 w-2 border border-blue-800 bg-blue-100" />
          N
        </span>
        <span className="inline-flex items-center gap-1">
